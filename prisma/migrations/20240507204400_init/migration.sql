...
CREATE TABLE `TaxFiling` (
    ...
    `status` ENUM('DRAFT', 'AWAITING_APPROVAL', 'APPROVED', 'SUBMITED') NOT NULL,
    `orgId` INTEGER NOT NULL,
    `extraordinary` BOOLEAN NOT NULL,
    `active` BOOLEAN NOT NULL DEFAULT true,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,
    `companyId` INTEGER NOT NULL,
    `taxFormId` INTEGER NOT NULL,
...
-- AddForeignKey
ALTER TABLE `TaxFiling` ADD CONSTRAINT `TaxFiling_taxFormId_fkey` FOREIGN KEY (`taxFormId`) REFERENCES `TaxForm`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `TaxForm` ADD CONSTRAINT `TaxForm_companyId_fkey` FOREIGN KEY (`companyId`) REFERENCES `Company`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
